...
<template>
  <div class="flex flex-wrap -mx-2 gap-y-20 gap-x-10 justify-around">
    <div v-for="card in filteredCards" :key="card.id" class="px-10 w-full  md:w-96  ">
      <n-card :title="card.title" class=" hover:shadow-2xl card-list" >
        <template #default>
          <img :src="card.image" alt="Card Image" class="card-image mb-4 absolute right-0 top-2 w-12 h-12">
          
          {{ card.details }}
        </template>
        <template #footer>
          <n-space>
            <n-tag v-for="tag in card.tags" :key="tag" type="info">{{ tag }}</n-tag>
          </n-space>
        </template>
        <template #action>
